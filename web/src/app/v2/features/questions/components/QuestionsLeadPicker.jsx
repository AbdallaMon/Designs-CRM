...
// Lead picker — the standalone-page entry for the LEAD-SCOPED questions surface. The route
// `/v2/questions` carries no lead context, so the user must first identify a lead; this
// component resolves a `clientLeadId` and lifts it to the page (which drives the URL `?leadId=`
// so the selection is shareable / embeddable). Mirrors salesStages/components/LeadPicker.jsx.
//
...
//   - lead.list absent   → a manual lead-id field (a user may still hold question.* for a lead
//     they own and arrive via a direct link; the server enforces the lead scope on read).
//
...
export function QuestionsLeadPicker({ selectedLeadId, onPick }) {
        ...
        <List dense disablePadding>
          {items.map((lead) => {
            ...
            return (
              ...
              >
                <ListItemText primary={`#${lead.id} — ${name}`} secondary={lead.status ?? ""} />
              </ListItemButton>
            );
          })}
        </List>
...
function ManualLeadId({ selectedLeadId, onPick }) {
  ...
  return (
    <Paper variant="outlined" sx={{ p: 2 }}>
      <Typography variant="subtitle1" sx={{ mb: 1.5 }}>
        أدخل رقم العميل لعرض أسئلة الجلسة
      </Typography>
...
export default QuestionsLeadPicker;
