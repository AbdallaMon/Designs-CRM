...
// selectors. The old generic `utilities/ids?model=&where=&select=` endpoint was
...
export function resolvePickListUrl({ model, where }) {
  switch (model) {
    case "Template":
      ...
      return `image-sessions/admin/templates/ids${
        where?.type ? `?type=${encodeURIComponent(where.type)}` : ""
      }`;
    case "Style":
      return `image-sessions/admin/style?notArchived=true`;
    case "Space":
      return `image-sessions/admin/space?notArchived=true`;
    default:
      // Fallback: the generic allow-listed pick-list (delegate names are lowercase-first).
      return `utilities/ids?model=${model}`;
  }
}
