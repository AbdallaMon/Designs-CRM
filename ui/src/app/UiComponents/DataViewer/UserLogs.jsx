...
import {
    ...
    Avatar,
    Grid2 as Grid,
    Container, Divider, CardHeader, DialogContent, IconButton, DialogTitle, Dialog, Button,
} from "@mui/material";
...
function UserLogs({staff,staffId}){
    ...
    return (
          <div>
              ...
              <Button variant="contained" onClick={handleClickOpen}>See {staff.name} logs</Button>
              <Dialog
                    fullScreen
                    open={open}
                    onClose={handleClose}
...
const Logs = ({staff,staffId}) => {
    ...
    const {
        data:notifications,
        loading,
    } = useDataFetcher(`admin/users/${staffId}/logs/`,false,)
    ...
    return (
          <Container maxWidth="xl" sx={{ marginY: 4 ,position:"relative"}}>
              ...
              <Card sx={{ boxShadow: 3, padding: 2 }}>
                  <CardHeader    title={`${staff.name} Logs for today`} subheader={`View ${staff.name} latest activity.`}>
                  </CardHeader>
