...
const LeadStatusChart = ({staff,staffId}) => {
    ...
    const [data,setData]=useState(Object.entries(leadStats.statuses).map(([key, value]) => ({
              ...
              <CardContent sx={{
                  ...
                  <Box sx={{ height: isMobile ? 350 : 400}}>
                      <ResponsiveContainer width="100%" height="100%"  minWidth="400px">
                          <BarChart
                                data={data}
                                margin={{ top: 20, right: isMobile ? 30 : 50, left: 20, bottom: isMobile ? 80 : 30 }}
                          >
                              ...
                              >
                                  {data.map((entry, index) => (
                                        <Cell
                                              key={`cell-${index}`}
                                              fill={STATUS_COLORS[entry.status.replace(' ', '_')] || COLORS[index % COLORS.length]}
                                        />
                                  ))}
                              </Bar>
                          </BarChart>
