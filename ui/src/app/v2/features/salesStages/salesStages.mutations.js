// Mutation runner for sales-stages write actions (advance / roll back the lead's sales stage).
// Wraps a salesStagesService call with a toast that resolves the backend message CODE → Arabic
// (NO hardcoded Arabic prose from responses; we resolve the language-neutral CODE the envelope
// carries). Routes through the service (no ad-hoc fetch) and the salesStagesMessages resolver.
// Returns the parsed envelope on success, or null on failure (after toasting the resolved
// error code). Mirrors calendar.mutations.js.

import { toast } from "react-toastify";
import { Success, Failed } from "@/app/v2/lib/toast/toastUtils";
import { resolveSalesStagesMessage } from "./config/salesStagesMessages.js";

/**
 * @param {() => Promise<object>} fn        a salesStagesService call returning the envelope
 * @param {object} [opts]
 ...
 */
export async function runSalesStagesMutation(
  fn,
