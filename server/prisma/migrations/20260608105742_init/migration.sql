...
CREATE TABLE `ClientLead` (
    ...
    `userId` INTEGER NULL,
    `status` ENUM('NEW', 'IN_PROGRESS', 'INTERESTED', 'NEEDS_IDENTIFIED', 'NEGOTIATING', 'LEADEXCHANGE', 'REJECTED', 'FINALIZED', 'CONVERTED', 'ON_HOLD', 'ARCHIVED') NOT NULL DEFAULT 'NEW',
    `telegramLink` VARCHAR(191) NULL,
    `selectedCategory` ENUM('CONSULTATION', 'DESIGN', 'OLDLEAD') NOT NULL,
    `type` ENUM('ROOM', 'BLUEPRINT', 'CITY_VISIT', 'APARTMENT', 'CONSTRUCTION_VILLA', 'UNDER_CONSTRUCTION_VILLA', 'PART_OF_HOME', 'COMMERCIAL', 'NONE') NOT NULL DEFAULT 'CONSTRUCTION_VILLA',
    `description` VARCHAR(191) NULL,
    `location` VARCHAR(255) NULL,
    `projectType` VARCHAR(255) NULL,
    `projectStage` VARCHAR(255) NULL,
    `previousWork` TEXT NULL,
    `hasArchitecturalPlan` VARCHAR(255) NULL,
    `serviceType` VARCHAR(255) NULL,
    `decisionMaker` VARCHAR(255) NULL,
    `bookingRequestStatus` ENUM('IN_PROGRESS', 'SUBMITTED') NOT NULL DEFAULT 'IN_PROGRESS',
    `bookingSubmittedAt` DATETIME(3) NULL,
    `clientDescription` TEXT NULL,
    `stripieMetadata` JSON NULL,
    `emirate` ENUM('DUBAI', 'ABU_DHABI', 'SHARJAH', 'AJMAN', 'UMM_AL_QUWAIN', 'RAS_AL_KHAIMAH', 'FUJAIRAH', 'KHOR_FAKKAN', 'OUTSIDE') NULL,
    `leadType` ENUM('NORMAL', 'CONVERTED') NOT NULL DEFAULT 'NORMAL',
    `previousLeadId` INTEGER NULL,
    `personality` ENUM('EXPRESSIVE', 'ANALYTICAL', 'INTROVERTED', 'DRIVER') NULL,
    `stateOfTheProject` TEXT NULL,
    `discoverySource` ENUM('INSTAGRAM', 'TIKTOK', 'TV', 'FACEBOOK', 'YOUTUBE', 'GOOGLE', 'INTERIOR_MAGAZINE_SITE', 'REFERRAL', 'OTHER') NULL,
    ...
    `ourCost` VARCHAR(191) NULL,
    `contractorCost` VARCHAR(191) NULL,
    `country` VARCHAR(191) NULL,
    `commissionCleared` BOOLEAN NOT NULL DEFAULT false,
    `initialConsult` BOOLEAN NOT NULL DEFAULT true,
    `assignedAt` DATETIME(3) NULL,
    `timeToContact` DATETIME(3) NULL,
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updatedAt` DATETIME(3) NOT NULL,
    `finalizedDate` DATETIME(3) NULL,
    `accountantId` INTEGER NULL,
    `accountantAssignedAt` DATETIME(3) NULL,

    UNIQUE INDEX `ClientLead_code_key`(`code`),
...
CREATE TABLE `User` (
    ...
    `isSuperSales` BOOLEAN NOT NULL DEFAULT false,
    `role` ENUM('ADMIN', 'STAFF', 'THREE_D_DESIGNER', 'TWO_D_DESIGNER', 'TWO_D_EXECUTOR', 'ACCOUNTANT', 'SUPER_ADMIN', 'SUPER_SALES', 'CONTACT_INITIATOR') NOT NULL DEFAULT 'STAFF',
    `allowNotification` BOOLEAN NOT NULL DEFAULT true,
    `allowEmailing` BOOLEAN NOT NULL DEFAULT true,
    `telegramUsername` VARCHAR(191) NULL,
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updatedAt` DATETIME(3) NOT NULL,
    `lastSeenAt` DATETIME(3) NULL,
    `maxLeadsCounts` INTEGER NULL DEFAULT 50,
    `maxLeadCountPerDay` INTEGER NOT NULL DEFAULT 5,
    `notAllowedCountries` JSON NULL,
    `profilePicture` TEXT NULL,
    `googleRefreshToken` TEXT NULL,
    `googleAccessToken` TEXT NULL,
    `googleTokenExpiresAt` DATETIME(3) NULL,
...
CREATE TABLE `Client` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    `name` VARCHAR(255) NOT NULL,
    `arName` VARCHAR(255) NULL,
    `enName` VARCHAR(255) NULL,
    `phone` VARCHAR(20) NOT NULL,
    `email` VARCHAR(255) NOT NULL,
    `contactAgreement` BOOLEAN NULL,
    `contactInitialPriceAgreement` BOOLEAN NULL,
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updatedAt` DATETIME(3) NOT NULL,
    `lastSeenAt` DATETIME(3) NULL,
...
CREATE TABLE `Note` (
    ...
    `userId` INTEGER NOT NULL,
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `notedUserId` INTEGER NULL,
    `baseEmployeeSalaryId` INTEGER NULL,
    `rentId` INTEGER NULL,
    `rentPeriodId` INTEGER NULL,
    `operationalExpensesId` INTEGER NULL,
    ...
    `sharedUpdateId` INTEGER NULL,
    `imageSessionId` INTEGER NULL,
    `selectedImageId` INTEGER NULL,
    `salesStageId` INTEGER NULL,
    `deliveryScheduleId` INTEGER NULL,

    INDEX `Note_taskId_idx`(`taskId`),
    INDEX `Note_clientLeadId_idx`(`clientLeadId`),
    INDEX `Note_userId_idx`(`userId`),
    INDEX `Note_baseEmployeeSalaryId_idx`(`baseEmployeeSalaryId`),
    INDEX `Note_rentId_idx`(`rentId`),
    INDEX `Note_rentPeriodId_idx`(`rentPeriodId`),
    INDEX `Note_operationalExpensesId_idx`(`operationalExpensesId`),
    INDEX `Note_paymentId_idx`(`paymentId`),
    INDEX `Note_invoiceId_idx`(`invoiceId`),
    INDEX `Note_commissionId_idx`(`commissionId`),
    INDEX `Note_sharedUpdateId_idx`(`sharedUpdateId`),
    INDEX `Note_updateId_idx`(`updateId`),
    INDEX `Note_imageSessionId_idx`(`imageSessionId`),
    INDEX `Note_selectedImageId_idx`(`selectedImageId`),
    INDEX `Note_contractId_idx`(`contractId`),
    INDEX `Note_salesStageId_idx`(`salesStageId`),
    INDEX `Note_notedUserId_idx`(`notedUserId`),
    INDEX `Note_deliveryScheduleId_idx`(`deliveryScheduleId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
