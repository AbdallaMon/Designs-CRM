...
**Remaining BE:** none — **all backend domain modules are migrated.** What's left before cutover: the deferred **clients** public-facing aggregator (`routes/clients/clients.js` — now unblocked; it mostly re-points to already-migrated client sub-flows: leads/payments/notes/languages/telegram/uploads + the already-done chat/calendar/contracts/image-session client routers), then a residual sweep for any stragglers, then the **FE** migration phase, then **Phase 12 cutover** (flip FE fully to `/v2`, retire legacy, rename `ui/→web/`). **FE:** deferred (features for the BE-only modules).
