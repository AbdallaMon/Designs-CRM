...
**Phase-2 flag purge (IN PROGRESS):** migrating the remaining logic reads off the flags onto profiles. Done: leads module (all sites) + FE `LeadSliderCard`. Remaining (tracked): backend logic in `users` (`user.usecase`/`user.repo`/`user.dto`), `projects` (`project.usecase`/`.repo`/`.dto`, `task.dto`), `dashboard`, `calendar/availability`, `utilities`, `generic-delete`, `contracts/legacy` scope read; FE central helpers (`checkIfAdminOrSuperSales`, `checkIfPrimaryStaff`, `checkIfAdminOrSuperOrContactInitiator`) + their remaining direct-flag call sites; and dropping the `isSuperSales`/`isPrimary` display exposure from `auth.dto` once the FE no longer reads them. Sanctioned flag reads to KEEP: `deriveProfilesFromLegacy`/`resolveProfileKey` (backfill), `setUserProfile` write-sync, and (transitional boundary) `auth.middleware` `legacyIsAdminTier`.
