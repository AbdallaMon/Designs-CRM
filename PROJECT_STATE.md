...
>
> Last updated: **2026-06-06** · Branch: `server-migration`
...
Commits on `server-migration`: foundation `3c84d5a` → chat `d980950` → site-utility `38f7bf0` → courses `1dbc181` → leads `c709d14` → users `5cf59ee` → validation-fix `934ba69` → projects `fe9957b`. App boots; legacy + `/v2` coexist (strangler). Full suite: **152 tests / 15 files green**.

**Modules done (BE):** Chat (+FE), site-utility (+FE), Courses/LMS, Leads/clientLead CORE (IDOR keystone), Users (unblocks chat's `/v2/users` directory), Projects domain (project+task+update+delivery; IDOR keystone for designers/executors). Each went through review → rework → verify. Security holes fixed in every module: Courses 2 critical IDOR; Leads 2 HIGH; Users profile-IDOR (full-row+password-hash leak + escalation) + mass-assign; Projects broad-delete IDOR + PII enumeration + mass-assign. Shared: validate middleware now emits a CODE not Zod prose.
**Remaining BE:** accounting, calendar, dashboard, notifications, clients, contracts (frozen PDF), image-sessions (frozen PDF), reviews, utilities, questions, sales-stages. **FE:** deferred (features for the BE-only modules).
